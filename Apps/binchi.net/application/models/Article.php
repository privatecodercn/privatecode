	...
	*/
	...
	public function save($data)
	{
		...
		if (empty($data['id']))
		{
			...
			$article = ORM::for_table('article')->find_one($data['id']);
			if (!$article->id)
			{
				return false;
			}
		...
		$result = ORM::for_table('pics')->raw_execute($sql);
		if (!$result)
		...
		if ($isNew)
		{
			$articleContent = ORM::for_table('article_content')->create();
			$articleContent->id = $article->id;
		} else {
			$articleContent = ORM::for_table('article_content')->find_one($article->id);
			if (!$articleContent)
			{
				$articleContent = ORM::for_table('article_content')->create();
				$articleContent->id = $article->id;
