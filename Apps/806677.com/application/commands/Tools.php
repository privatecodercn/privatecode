...
class ToolsCmd
{
    public function t() 
    {
        ...
        $connection = 'mysql:host='.$dbCfg->host.';dbname=live_broadcast;charset='.$dbCfg->charset;
        Db::configure($connection);
        Db::configure('username', $dbCfg->username);
        Db::configure('password', $dbCfg->password);
        
        $db = Db::table('lb_content');
        for ($i=0; $i<10000; $i++) {
            $data = [
                'lb_id'       => 21,
                'create_time' => mt_rand(1390023423, time()),
                'content'     => $this->getStr(rand(50, 200)),
            ];
