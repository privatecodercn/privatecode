...
class Page
{
	 ...
	 * return boolean
	 */
	public static function init($size=10, $rows=0, $page=null)
	{
		if ($size>0) {
			self::$size = $size;
		} elseif($size==0) {
	...
	public static function setPage($page=null) {
		if (is_null($page))
		{
			if (isset($_REQUEST[self::$pageVar]))
			{
		...
		if (self::$page < 1) {
			...
			$last = self::getPageUrl(self::$total);
	...
	public static function parseQs()
	{
		...
		if (!empty($qs))
		{
			self::$firstUrl = $arr['path'].'?'.$qs;
			$qs .= '&';
		} else {
			self::$firstUrl = $arr['path'];
		}
		self::$qsTpl = urldecode($qs.self::$pageVar.'={page}');
		self::$urlTpl = $arr['path'].'?'.self::$qsTpl;
	}
	...
	{
		return str_replace('{page}', $page, self::$urlTpl);
	}
	...
	public static function getNavPageTurn()
	{
		...
		if (self::$page > 1){
			$page_pre = '<a href="'.self::getPageUrl(self::$page - 1).'">【上一页】</a>&nbsp;';
		}
		if (self::$page < self::$total) {
			$page = self::$page<self::$total ? (self::$page + 1) : self::$total;
			$page_next ='<a href="'.self::getPageUrl($page).'">【下一页】</a>&nbsp;';
		}
		if (self::$page != self::$total) {
			$page_last = '<a href="'.self::getPageUrl(self::$total).'">【末页】</a>&nbsp;';
		}
	...
	public static function getPageCodeTurn()
	{
		...
		if (self::$page != self::$total)
		{
			...
			$str .= '<a href="'.self::getPageUrl(self::$total).'" '
	 ...
	 */
	public static function getPageByView($viewFile)
	{
		include $viewFile;
	}
}
