...
class Mysql
{
	...
	*/
	 ...
	 * @return int | false
	 */
	public function plus($table, $data, $where, $param=null)
	{
		$set_list = array();
		is_null($param) && $param=array();
		if (is_array($data)) {
	...
	public function getMaxId($table)
	{
		...
		$arr = $this->fetchFromSql($sql, false, null, PDO::FETCH_COLUMN);
		return $arr;
	}
	...
	public function buildSql($table, $options=null)
	{
		...
		if (empty($options['order'])) {
			...
			$order = ' order by '.join(',', $options['order']);
		} else {
