 ...
 * Created		: 2009-07-12
 * Modified		: 2012-09-12
 * @link		: http://www.clq.name/
 * @copyright	: © 2009-2012 InPHP.Com
 * @version		: 3.0.0
...
class Db2
{
	...
	*/
	 ...
	 * @return int | false
	 */
	public static function plus($table, $data, $where=null, $param=null)
	{
		$ret = self::_db()->plus($table, $data, $where, $param);
		if (!$ret) {
			self::$errors = self::_db()->errors;
		}
		return $ret;
	 ...
	 * @param string|array $options
	 */
	public static function fetch($table, $options=array())
	{
		if (is_string($options)) {
			$options = array(
				'where'	=> $options
		...
		$ret = self::_db()->fetchFromSql($sql, false, $param);
		if (!$ret) {
			self::$errors = self::_db()->errors;
		}
		return $ret;
	...
	{
		return self::_db()->getMaxId($table);
	}
	...
	{
		return self::_db()->isTableExist($table);
	}
